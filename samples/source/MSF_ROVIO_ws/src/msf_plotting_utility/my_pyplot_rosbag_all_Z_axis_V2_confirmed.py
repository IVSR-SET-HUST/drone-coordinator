...
for topic, msg, t in bag.read_messages(topics=topic_rovio_only):
	if topic == "/rovio/pose_with_covariance_stamped":
		tempdata1 = np.append(tempdata1, [[msg.header.stamp.secs+1e-9*msg.header.stamp.nsecs-offset1],[msg.pose.pose.position.z]], axis=1)
...
for topic, msg, t in bag2.read_messages(topics=topic_msf):
	if topic == "/msf_core/state_out":
		tempdata2 = np.append(tempdata2, [[msg.header.stamp.secs+1e-9*msg.header.stamp.nsecs-offset2],[msg.data[2]]], axis=1)
...
for topic, msg, t in bag3.read_messages(topics=topic_msf):
	if topic == "/msf_core/state_out":
		tempdata3 = np.append(tempdata3, [[msg.header.stamp.secs+1e-9*msg.header.stamp.nsecs-offset3],[msg.data[2]]], axis=1)
...
with open(filename_groundtruth) as f:
    ...
    highs = []
    for row in reader:
        tempdata4 = np.append(tempdata4, [[(float(row[0])-1.40371588837906E+018)/1000000000+1.62],[float(row[3])]], axis=1)
