...
for topic, msg, t in bag.read_messages(topics=topic_rovio_only):
	if topic == "/rovio/pose_with_covariance_stamped":
		tempdata1 = np.append(tempdata1, [[msg.header.stamp.secs+1e-9*msg.header.stamp.nsecs-offset1],[-msg.pose.pose.position.x]], axis=1)
...
for topic, msg, t in bag2.read_messages(topics=topic_msf):
	if topic == "/msf_core/state_out":
		tempdata2 = np.append(tempdata2, [[msg.header.stamp.secs+1e-9*msg.header.stamp.nsecs-offset2],[msg.data[0]]], axis=1)
...
for topic, msg, t in bag3.read_messages(topics=topic_msf):
	if topic == "/msf_core/state_out":
		tempdata3 = np.append(tempdata3, [[msg.header.stamp.secs+1e-9*msg.header.stamp.nsecs-offset3],[msg.data[0]]], axis=1)
