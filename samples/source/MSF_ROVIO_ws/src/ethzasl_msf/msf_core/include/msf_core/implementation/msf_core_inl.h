...
namespace msf_core
{
  ...
  MSF_Core<EKFState_T>::MSF_Core(const MSF_SensorManager<EKFState_T> &GetUserCalc)
      : usercalc_(GetUserCalc)
  { // The interface for the user to customize EKF
    ...
    if (currentState->time - lastState->time < -0.01 && predictionMade_)
    {
      ...
      MSF_ERROR_STREAM(
          __FUNCTION__ << "latest IMU message was out of order by a too large amount, "
                       ...
                       << msf_core::timehuman(lastState->time)
                       << " "
                                            ...
                                            shared_ptr<EKFState_T> &state_new)
  {
    ...
    OmegaMean *= 0.5 * dt;
    for (int i = 1; i < 5; i++)
    { // Can be made fourth order or less to save cycles.
      div *= i;
    ...
    dv = (state_new->template Get<StateDefinition_T::q>().toRotationMatrix() *
              ea +
          state_old->template Get<StateDefinition_T::q>().toRotationMatrix() *
              eaold) /
         2;
    state_new->template Get<StateDefinition_T::v>() =
        state_old->template Get<StateDefinition_T::v>() + (dv - constants::GRAVITY) * dt;
    state_new->template Get<StateDefinition_T::p>() =
        state_old->template Get<StateDefinition_T::p>() + ((state_new->template Get<StateDefinition_T::v>() + state_old->template Get<StateDefinition_T::v>()) / 2 * dt);
  }
                    ...
                    MSF_Core<EKFState_T>::nErrorStatesAtCompileTime> &F)
  {
    ...
    typedef Eigen::Matrix<double, MSF_Core<EKFState_T>::nErrorStatesAtCompileTime,
                          MSF_Core<EKFState_T>::nErrorStatesAtCompileTime>
        F_type;
    F = F_type::Identity();
    for (; it != itend; ++it)
    {
      ...
      shared_ptr<MSF_MeasurementBase<EKFState_T>> measurement)
  {
    ...
    MSF_INFO_STREAM(
        "Core parameters: " << std::endl
                            << "\tfixed_bias:\t" << usercalc_.GetParamFixedBias() << std::endl
                            << "\tfuzzythres:\t" << usercalc_.GetParamFuzzyTrackingThreshold() << std::endl
    ...
    MSF_INFO_STREAM("Core init with state: " << std::endl
                                             << state->Print());
    initialized_ = true;
      ...
      shared_ptr<MSF_MeasurementBase<EKFState_T>> measurement)
  {
    ...
    if (measurement->time < stateBuffer_.GetFirst()->time)
    {
      MSF_WARN_STREAM(
          ...
          "[measurement: "
          << timehuman(measurement->time) << " (s) first state in "
                                             "buffer: "
          << timehuman(stateBuffer_.GetFirst()->time) << " (s)]");
